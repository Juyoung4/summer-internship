...
<script type="text/javascript">
	...
	function fn_deleteYn(){
		...
		ajaxFormExecute("noticeFrm", "/five/ajaxNoticeDelete", "post", false, false, ajaxReturnDelete);
	}
	...
	function fn_list(){
		$("#noticeFrm").prop("action", "/five/notice");
		$('#nowPage').val('1');
 		formSubmit("noticeFrm");
	}
	
	function fn_view(noticeSeq){
		$("#noticeFrm").prop("action", "/five/noticeView");
		$('#noticeSeq').val(noticeSeq);
 		formSubmit("noticeFrm");
	}
	
	function fn_write(){
		location.href="/five/noticeWrite";	
	}
		...
		<div class="row">
			<div class="col-md-12 col-sm-12 col-xs-12">
				...
				<nav aria-label="Page navigation example" class="content-center text-center">
					<@pagingTag.Paging url="/five/notice" pageCount="${pageInfo.pageCount!}" rowCount="${pageInfo.rowCount!}" rowMax="${pageInfo.rowMax!}" nowPage="${pageInfo.nowPage!}" params="" />
				</nav>
			</div>
		</div>
	</div>
