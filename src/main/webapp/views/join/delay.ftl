...
<script type="text/javascript">
	...
	function fn_list(){
		$("#noticeFrm").prop("action", "/join/delay");
		$('#nowPage').val('1');
	...
	function fn_DapproveYn(){
		...
		ajaxFormExecute("noticeFrm", "/join/ajaxDUserApprove", "post", false, false, ajaxReturnDApprove);
	}
	...
	<div class="">
    	<div class="page-title">
  			<div class="title_left">
				<ul class="tabs">
  					<li class="tab-link" data-tab="tab-1"><a href='/join/admin'>승인 대기</a></li>
					<li class="tab-link current" data-tab="tab-2"><a href='/join/delay'>반려</a></li>
  				</ul>
			</div>
		</div>
		...
		<div id="tab-2" class="row">
			<div class="col-md-12 col-sm-12 col-xs-12">
				...
				<nav aria-label="Page navigation example" class="content-center text-center">
					<@pagingTag.Paging url="/join/delay" pageCount="${pageInfo.pageCount!}" rowCount="${pageInfo.rowCount!}" rowMax="${pageInfo.rowMax!}" nowPage="${pageInfo.nowPage!}" params="" />
				</nav>
			</div>
		</div>
	</div>
