...
function appendAjaxLoading(){
	var loadingHtml = '<div id="ajax-loading" style="position:fixed;left:0;right:0;top:0;bottom:0;z-index:100;background: rgba(0,0,0,0.2);">\n';
	loadingHtml += '<div style="position:fixed;top:50%;left:50%;margin-left:-21px;margin-top:-21px;"><img id="ajaxLoadingImg" src="/images/common/loading.gif" style="width:45px;"/></div>\n';
	loadingHtml += '</div>\n';
...
var jsAlert = {
	...
	confirm : function(title, msg, trueCallback, falseCallback, okBtnNm, cancelBtnNm) {
		...
		jConfirm(msg, title, okBtnNm, cancelBtnNm, data, function(result) {
			if (result) {
...
function getCommonCodeMstForGrp(grpCod, callback){
	$.ajax({
		url : "/common/getCommonCodeMstForGrp",
		type: "post",
		data: "grpCod="+grpCod,
		//contentType: false,
		processData: false,
